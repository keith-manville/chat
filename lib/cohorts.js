...
function createCohort({ workspaceId = defaultWorkspaceId, scenarioId, name }) {
  if (!scenarioId) throw new Error('scenarioId required');
  if (!name) name = 'Cohort ' + new Date().toISOString().slice(0, 16).replace('T', ' ');
  // Generate a unique code (retry up to 10 times in the unlikely collision case).
  let joinCode;
  for (let i = 0; i < 10; i++) {
    joinCode = generateJoinCode();
    const taken = db.prepare('SELECT 1 FROM cohorts WHERE join_code = ?').get(joinCode);
    if (!taken) break;
  ...
  db.prepare(
    `INSERT INTO cohorts (id, workspace_id, scenario_id, name, join_code, created_at, started_at)
     VALUES (?, ?, ?, ?, ?, ?, ?)`
  ).run(id, workspaceId, scenarioId, name, joinCode, now, now);
  // Materialize cohort channels.
